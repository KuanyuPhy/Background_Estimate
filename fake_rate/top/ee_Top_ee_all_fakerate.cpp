...
void ee_Top_ee_all_fakerate(TString file = "tmp.root", TString outputfile = "output.root")
{
    TFile *Topfile = TFile::Open(file);

    // For ntrk
    TH1D *h_Top_nTrk_lowDilepPt = Topfile->Get<TH1D>("h_Top_nTrk_jet_lowDilepPt");
    TH1D *h_Top_nTrk_cut_lowDilepPt = Topfile->Get<TH1D>("h_Top_nTrk_jet_cut_lowDilepPt");
    TH1D *h_Top_nTrk_highDilepPt = Topfile->Get<TH1D>("h_Top_nTrk_jet_highDilepPt");
    TH1D *h_Top_nTrk_cut_highDilepPt = Topfile->Get<TH1D>("h_Top_nTrk_jet_cut_highDilepPt");
...
int main(int argc, char **argv)
{
    if (argc == 1)
    {
        ee_Top_ee_all_fakerate();
    }
    else if (argc == 3)
    {
        ee_Top_ee_all_fakerate(argv[1], argv[2]);
    }
    else
    {
        cout << "argc" << argc << endl;
