...
vector<double> v_xs_Weight = {merge_weight.ee_TTTo2L2NuWeight, merge_weight.ee_ST_tW_topWeight,
                              merge_weight.ee_ST_tW_antitopWeight, merge_weight.ee_TTWJetsToLNuWeight, merge_weight.ee_TTWJetsToQQWeight,
                              merge_weight.ee_TTZToQQWeight, merge_weight.ee_TTZToLLNuNuWeight};
...
void ee_Top_ee_all(TString file = "tmp.root", TString outputfile = "output.root")
{
    ...
    for (int evt = 0; evt < T_Top_tree->GetEntries(); evt++)
    {
        ...
        //-------------------------------------------------------------
        // Jet var : different flavor  nTracks (jetflavor, jetpt, jeteta, jetalpha, jetntrk)
        //-------------------------------------------------------------
        ...
        {
            v_thinjet.push_back(ThinJet((*v_Top_Jethadronflavor)[i], (*v_Top_JetPT)[i], (*v_Top_JetEta)[i], (*v_Top_alpha)[i], (*v_Top_nTrack)[i], (*v_Top_JetMass)[i]));
        }
        ...
        if (f_Top_met > METcut)
        {
            ...
            } // High Dilepton PT scope
        }     // MET greater 140 GeV scope
    }         // End of Top loop
