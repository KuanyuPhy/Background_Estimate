...
int get_trk_binfakerate(float tmp_ntrk)
{
    int bin_pos_info;
...
int get_jetpt_binfakerate(float tmp_jetPt)
{
    int jetPtbin_pos_info;
    if (tmp_jetPt < 30)
    {
        jetPtbin_pos_info = 1;
    }
    else if (tmp_jetPt >= 30 && tmp_jetPt < 60)
    {
        jetPtbin_pos_info = 2;
    }
    else if (tmp_jetPt >= 60 && tmp_jetPt < 90)
    {
        jetPtbin_pos_info = 3;
    }
    else if (tmp_jetPt >= 90 && tmp_jetPt < 120)
    {
        jetPtbin_pos_info = 4;
    }
    else if (tmp_jetPt >= 120 && tmp_jetPt < 150)
    {
        jetPtbin_pos_info = 5;
    }
    else if (tmp_jetPt >= 150 && tmp_jetPt < 210)
    {
        jetPtbin_pos_info = 6;
    }
    else if (tmp_jetPt >= 210 && tmp_jetPt < 270)
    {
        jetPtbin_pos_info = 7;
    }
    else if (tmp_jetPt >= 270 && tmp_jetPt < 350)
    {
        jetPtbin_pos_info = 8;
    }

    else if (tmp_jetPt >= 350 && tmp_jetPt < 450)
    {
        jetPtbin_pos_info = 9;
    }
...
void Ratio_Top_apply_LO(TString file = "/home/kuanyu/Documents/root_file/BgEstimation/top_TTTo2L2Nu_2.root", TString outputfile = "output.root")
{
    ...
    //----------------------------
    // Apply fakerate(jetPT)
    //----------------------------
    TH1D *h_Top_trk_LO_byjetPTbin_CR = new TH1D("h_Top_trk_LO_byjetPTbin_CR", "", Ntrk_Nbins, Ntrk_edges);
    h_Top_trk_LO_byjetPTbin_CR->Sumw2();

    TH1D *h_Top_JetPt_LO_byjetPTbin_CR = new TH1D("h_Top_JetPt_LO_byjetPTbin_CR", "", NJet_Nbins, NJet_edges);
    h_Top_JetPt_LO_byjetPTbin_CR->Sumw2();

    TH1D *h_Top_JetEta_LO_byjetPTbin_CR = new TH1D("h_Top_JetEta_LO_byjetPTbin_CR", "", JetEta_Nbins, JetEta_low_bound, JetEta_upper_bound);
        ...
        if (f_Top_met > METcut)
        {
            if (f_Top_dileppt >= DilepPTcut)
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    h_Top_trk_LO_bybin_CR->Fill(v_thinjet[i].GetNtrk(), Top_weight);
                    h_Top_JetPt_LO_bybin_CR->Fill(v_thinjet[i].GetPt(), Top_weight);
                    h_Top_JetEta_LO_bybin_CR->Fill(v_thinjet[i].GetEta(), Top_weight);

                    // For fakerate(jetPT) result
                    h_Top_trk_LO_byjetPTbin_CR->Fill(v_thinjet[i].GetNtrk(), Top_weight);
                    h_Top_JetPt_LO_byjetPTbin_CR->Fill(v_thinjet[i].GetPt(), Top_weight);
                    h_Top_JetEta_LO_byjetPTbin_CR->Fill(v_thinjet[i].GetEta(), Top_weight);

                    // For fakerate(jetETa) result
                    h_Top_trk_LO_byjetEtabin_CR->Fill(v_thinjet[i].GetNtrk(), Top_weight);
                    h_Top_JetPt_LO_byjetEtabin_CR->Fill(v_thinjet[i].GetPt(), Top_weight);
                    h_Top_JetEta_LO_byjetEtabin_CR->Fill(v_thinjet[i].GetEta(), Top_weight);
